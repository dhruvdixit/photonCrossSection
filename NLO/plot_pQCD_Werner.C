...
void compare_scale()
{
    for (int isys = 0; isys < nsys; ++isys)
    {
		...
		TLegend* leg = new TLegend(0.62,0.50,0.85,0.65);
                leg->SetBorderSize(0);
                ...
                if (isys==1 && ibeamE==0) 
                {
                    h_5TeV_pPb->SetMarkerStyle(25);
                    h_5TeV_pPb->SetMarkerSize(1);
                    //h_5TeV_pPb->Draw("psame");
                    leg_data->AddEntry(h_5TeV_pPb,"w/o NL","p");
                    
                    h_5TeV_wNL_pPb->SetMarkerSize(1);
                    h_5TeV_wNL_pPb->Draw("psame");
                    leg_data->AddEntry(h_5TeV_wNL_pPb,"w/ NL","p");
                }
                if (isys==1 && ibeamE==1) 
                {
                    h_8TeV_pPb->SetMarkerSize(1);
                    h_8TeV_pPb->Draw("psame");
                }
                if (isys==0 && ibeamE==0)
                {
                ...
                for (int iscale = 0; iscale < nscale; ++iscale)
                {
                    ...
                    else
                    {
                        ...
                        TF1* fit_func = new TF1("fit_func","[0]/(pow(exp(-[1]*x)+x/[2],[3]))",0.0,100);
                        fit_func->SetParameter(0,1E3);
                        fit_func->SetParameter(1,0.5);
                        fit_func->SetParameter(2,0.5);
                        fit_func->SetParameter(3,8);
                        // if (ibeamE==0) g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Fit(fit_func,"R","",40,60);
                    }
                }
                leg->Draw("same");
                leg_data->Draw("same");
                ...
                tl->SetTextAlign(11);
                tl->SetTextSize(0.04);
                tl->DrawLatexNDC(0.62,0.66,Form("%s %.2f TeV",sys_name[isys],beamE[ibeamE]));

		TLatex* tl_pQCD = new TLatex();
                tl_pQCD->SetTextAlign(11);
                tl_pQCD->SetTextSize(0.032);
		tl_pQCD->DrawLatexNDC(0.62,0.86,"ALICE Preliminary");
                tl_pQCD->DrawLatexNDC(0.62,0.82,"NLO pQCD");
                tl_pQCD->DrawLatexNDC(0.62,0.78,"(by W. Vogelsang)");
                tl_pQCD->DrawLatexNDC(0.62,0.74,"NNPDF3.0 PDF");
                tl_pQCD->DrawLatexNDC(0.62,0.70,"GRV FF");
		
                gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_%s_scale_comparison.pdf\")", cno-1, sys_abbr[isys], beamE_abbr[ibeamE]) );
            }
            ...
            for (int iscale = 0; iscale < nscale; ++iscale)
            {
                ...
                if (isys==1 && ibeamE==1) nbins = h_8TeV_pPb->GetNbinsX();
                if (isys==0 && ibeamE==0) nbins = h_5TeV_pp->GetNbinsX();
                if (isys==0 && ibeamE==1) nbins = h_8TeV_pp->GetNbinsX();
                for (int ibin = 1; ibin < nbins+1; ++ibin)
                {
                    double temp_center = h_5TeV_pPb->GetXaxis()->GetBinCenter(ibin);
                    double temp_width = h_5TeV_pPb->GetXaxis()->GetBinWidth(ibin);
                    if (isys==1 && ibeamE==1) 
                    {
                        temp_center = h_8TeV_pPb->GetXaxis()->GetBinCenter(ibin);
                        temp_width = h_8TeV_pPb->GetXaxis()->GetBinWidth(ibin);
                    }
                    if (isys==0 && ibeamE==0) 
                    {
                        temp_center = h_5TeV_pp->GetXaxis()->GetBinCenter(ibin);
                        temp_width = h_5TeV_pp->GetXaxis()->GetBinWidth(ibin);
                    }
                    if (isys==0 && ibeamE==1) 
                    {
                    ...
                    else average_calculation = average_pQCD_calculation(bin_edge_left,bin_edge_right,g_pQCD_nPDF_sum_pPb[ibeamE][iscale]);                    
                    
                    if (flag_use_average)
                    {
                        if (isys==1 && ibeamE==0) ratio_scale[ratio_npt] = h_5TeV_wNL_pPb->GetBinContent(ibin)/average_calculation;
                        if (isys==1 && ibeamE==1) ratio_scale[ratio_npt] = h_8TeV_pPb->GetBinContent(ibin)/average_calculation;
                    ...
                    {
                        if (isys==1 && ibeamE==0) ratio_scale[ratio_npt] = h_5TeV_wNL_pPb->GetBinContent(ibin)/g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Eval(ratio_pt[ratio_npt]);
                        if (isys==1 && ibeamE==1) ratio_scale[ratio_npt] = h_8TeV_pPb->GetBinContent(ibin)/g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Eval(ratio_pt[ratio_npt]);
		  ...
		  cout << "pQCD_NLO_pQCD_ratio " << sys_abbr[isys] << " " << beamE_abbr[ibeamE] << " " << scale[iscale] << endl; 
		  for(int ipoint = 0; ipoint < g_pQCD_data_ratio[iscale]->GetN(); ipoint++) {
		    cout << "X[" << ipoint << "] = " << (g_pQCD_data_ratio[iscale]->GetX())[ipoint] << "\t"
			 << "Y[" << ipoint << "] = " << (g_pQCD_data_ratio[iscale]->GetY())[ipoint]
			 << endl;
		  }
		...
		tl_pQCD->DrawLatexNDC(0.62,0.82,"NLO pQCD");
                ...
                gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_pQCD_%s_%s_ratio.pdf\")", cno-1, sys_abbr[isys], beamE_abbr[ibeamE]) );
		gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_pQCD_%s_%s_ratio.C\")", cno-1, sys_abbr[isys], beamE_abbr[ibeamE]) );
...
void compare_component()
{
    for (int isys = 0; isys < nsys; ++isys)
    {
        for (int ibeamE = 0; ibeamE < nbeamE; ++ibeamE)
        {
            ...
            mc(cno++);
            {
                ...
                float plot_yrange_lo = 0.5*TMath::MinElement(npt,pQCD_nPDF_direct_pPb[ibeamE][iscale]);
                float plot_yrange_hi = 1.5*TMath::MaxElement(npt,pQCD_nPDF_direct_pPb[ibeamE][iscale]);
                ...
                {
                    g_pQCD_nPDF_frag_pPb[ibeamE][iscale]->SetLineColor(kRed);
                    g_pQCD_nPDF_direct_pPb[ibeamE][iscale]->SetLineColor(kBlue);
                    g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->SetLineColor(kBlack);
                    g_pQCD_nPDF_frag_pPb[ibeamE][iscale]->Draw("csame");
                    g_pQCD_nPDF_direct_pPb[ibeamE][iscale]->Draw("csame");
                    g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Draw("csame");
                    leg->AddEntry(g_pQCD_nPDF_sum_pPb[ibeamE][iscale],"Isolated #gamma","l");
                    leg->AddEntry(g_pQCD_nPDF_direct_pPb[ibeamE][iscale],"Direct #gamma","l");
                    leg->AddEntry(g_pQCD_nPDF_frag_pPb[ibeamE][iscale],"Frag #gamma","l");
                }
                leg->Draw("same");
                ...
                gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_%s_component_comparison.pdf\")", cno-1, sys_abbr[isys], beamE_abbr[ibeamE]) );
            }
        }
    }
    ...
    for (int ipt = 0; ipt < npt; ++ipt)
    {
        ...
        {
          ratio_frag[ipt] = pQCD_nPDF_frag_pPb[ibeamE][iscale][ipt]/pQCD_nPDF_sum_pPb[ibeamE][iscale][ipt];
          ratio_direct[ipt] = pQCD_nPDF_direct_pPb[ibeamE][iscale][ipt]/pQCD_nPDF_sum_pPb[ibeamE][iscale][ipt];
        } 
    }
    ...
    mcs(cno++);
    {
        ...
        gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_%s_component_ratio.pdf\")", cno-1, sys_abbr[isys], beamE_abbr[ibeamE]) );
    }
}
...
void compare_sys()
{
    ...
    mc(cno++);
    {
        ...
        float plot_yrange_lo = 0.5*TMath::MinElement(npt,pQCD_nPDF_sum_pPb[ibeamE][iscale]);
        float plot_yrange_hi = 1.5*TMath::MaxElement(npt,pQCD_nPDF_sum_pPb[ibeamE][iscale]);
        ...
        for (int isys = 0; isys < nsys; ++isys)
        {
            if (isys==0)
            {
                g_pQCD_sum_pp[ibeamE][iscale]->SetLineColor(kRed);
                g_pQCD_sum_pp[ibeamE][iscale]->Draw("csame");
                leg->AddEntry(g_pQCD_sum_pp[ibeamE][iscale],Form("%s",sys_name[isys]),"l");
            }
            else
            {
                g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->SetLineColor(kBlue);
                g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Draw("csame");
                leg->AddEntry(g_pQCD_nPDF_sum_pPb[ibeamE][iscale],Form("%s",sys_name[isys]),"l");
            }
        }
        leg->Draw("same");
        ...
        tl->SetTextAlign(11);
        tl->SetTextSize(0.04);
        tl->DrawLatexNDC(0.2,0.83,Form("%.2f TeV",beamE[ibeamE]));

        gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_sys_comparison.pdf\")", cno-1, beamE_abbr[ibeamE]) );
    }
}
...
void compare_beamE()
{
    ...
    mc(cno++);
    {
        ...
        for (int ibeamE = 0; ibeamE < nbeamE; ++ibeamE)
        {
            ...
            {
                g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->SetLineColor(beamE_color[ibeamE]);
                g_pQCD_nPDF_sum_pPb[ibeamE][iscale]->Draw("csame");
                leg->AddEntry(g_pQCD_nPDF_sum_pPb[ibeamE][iscale],Form("%.2f TeV",beamE[ibeamE]),"l");
            }
        }
        leg->Draw("same");
        ...
        gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_beamE_comparison.pdf\")", cno-1, sys_abbr[isys]) );
    }
    ...
    for (int ipt = 0; ipt < npt; ++ipt)
    {
        ...
        else ratio_beamE[ipt] = pQCD_nPDF_sum_pPb[1][iscale][ipt]/pQCD_nPDF_sum_pPb[0][iscale][ipt];
    }
    ...
    mcs(cno++);
    {
        ...
        gROOT->ProcessLine( Form("cc%d->Print(\"pQCD_NLO_%s_beamE_ratio.pdf\")", cno-1, sys_abbr[isys]) );
    }
}
...
void set_real_data()
{
    ...
    TFile* fin4 = new TFile("/global/homes/d/ddixit/photonCrossSection/8TeVResults/pp/yield_8TeVpp.root");
    h_8TeV_pPb = (TH1F*)fin1->Get("hEG1_8TeVCS_pPb");
    //h_8TeV_pp = (TH1F*)fin1->Get("hEG1-8TeVCS_pp");
    h_8TeV_pp = (TH1F*)fin4->Get("h0");
    ...
    // TFile* fin = new TFile("../pPbCompairison8TeV5TeV_RF991.root");
    // fin->ls();
    // h_8TeV_pPb = (TH1F*)fin->Get("hEG1_8TeVCS");
    // h_5TeV_pPb = (TH1F*)fin->Get("hEG1_5TeVCS");

//     double Ncoll_pPb_5TeV = 6.723;
//     double Ncoll_pPb_8TeV = 7.085;
//     h_5TeV_pPb->Scale(1/Ncoll_pPb_5TeV);
//     h_8TeV_pPb->Scale(1/Ncoll_pPb_8TeV);
}

void plot_pQCD_Werner()
{
    set_real_data();
