...
const int nsys = 2;
const char* sys_abbr[nsys] = {"pp", "pPb"};
const float nucleon[nsys] = {1, 207};
...
void input_pQCD_Werner()
{
    for (int isys = 0; isys < nsys; ++isys)
    {
        for (int ibeamE = 0; ibeamE < nbeamE; ++ibeamE)
        {
            ...
            {
                ifstream fin(Form("alice_%s_sc%s_%s.dat",beamE_abbr[ibeamE],scale_abbr[iscale],sys_abbr[isys]));
    ...
    fout << "const int nsys = 2;" << endl;
    fout << "const char* sys_name[nsys] = {\"p+p\", \"p+Pb\"};" << endl;
    fout << "const char* sys_abbr[nsys] = {\"pp\", \"pPb\"};" << endl;
    fout << "const float nucleon[nsys] = {1, 207};" << endl;
    fout << endl;
