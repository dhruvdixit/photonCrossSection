...
void Run(const int TrackBit, TString address, bool nonLinCorrOn = false, bool hasAliDir = true, bool triggered = true){
 ...
 //////////////////////////////////////////////////////////////////////////////////*/
  ...
  for (Long64_t ievent=0;ievent< numEntries ;ievent++) {
    ...
    //if(TMath::Abs(primary_vertex[2]) > 10.0) {numEvents_Zmore10++;}
	...
	if( ((cluster_e_cross[n]/clusterE)>0.03))                             clusterCutBits |= (1 << 1); clusterCutPassed |= (1 << 1);//removes "spiky" clusters
	if( (cluster_nlocal_maxima[n]<= 2))                                     clusterCutBits |= (1 << 2); clusterCutPassed |= (1 << 2);//require to have at most 2 local maxima.
	if( (cluster_distance_to_bad_channel[n]>=2.0))                          clusterCutBits |= (1 << 3); clusterCutPassed |= (1 << 3);//distnace to bad channels
	if( ((cluster_tof[n] > -30) && (cluster_tof[n]) < 30))                  clusterCutBits |= (1 << 4); clusterCutPassed |= (1 << 4);//distnace to bad channels
	//Isolation and shower shape selection:
	if( (isolation < 2))                                                    clusterCutBits |= (1 << 5); clusterCutPassed |= (1 << 5);//isolation r = 0.4 and energy < 2
	//if( (cluster_iso_its_04[n]  < 2))                                       clusterCutBits |= (1 << 5); clusterCutPassed |= (1 << 5);//isolation r = 0.4 and energy < 2
	if( (ptDepShowerShapeCut(clusterPt, cluster_lambda_square[n][0])))  clusterCutBits |= (1 << 6); clusterCutPassed |= (1 << 6);//single-photon selection, not merged
