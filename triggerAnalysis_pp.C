...
void Run(const int TrackBit, TString address, bool nonLinCorrOn = false, bool hasAliDir = true, bool triggered = true){
 ...
 //////////////////////////////////////////////////////////////////////////////////*/
  ...
  ULong64_t trigMask_17q_trigs1[numTrigs] = {0};//0 = MB, 1 = DG2, 2 = EG2, 3 = EJ2, 4 = EJ1, 
  ...
  for (Long64_t ievent=0;ievent< numEntries ;ievent++) {
    ...
    bool isMB, isDG2, isEG2, isEJ1, isEJ2;
    isMB = isDG2 = isEG2 = isEJ1 = isEJ2 = false;
    ...
    if(not( TMath::Abs(primary_vertex[2])<10)){
	hEventCut->Fill(2);
    ...
    //Loop over clusters
    bool one12GeVcluster = false;
    for(ULong64_t n=0; n< ncluster; n++){
      if(cluster_pt[n] > 12 ) {
	one12GeVcluster = true;
	//break;
      } //select pt of photons
    }//end cluser check
    ...
    if(!one12GeVcluster){
      ...
      if(isDG2) hEventCut_DG2->Fill(5);
      if(isEG2) hEventCut_EG2->Fill(5);
      //if(isEJ1) hEventCut_EJ1->Fill(5);
      //if(isEJ2) hEventCut_EJ2->Fill(5);
      
      continue;
    }//no cluster with E > 12 GeV*/
  ...
  cout << "total/minbias/DG2/EG2/EJ1/EJ2 events:" << endl;
  cout << "before:" << "\t" <<
    numEvents_passTrig << "\t" << numEvents_MB_before << "\t" <<
    numEvents_DG2_before << "\t" << numEvents_EG2_before << "\t" <<
    numEvents_EJ1_before << "\t" << numEvents_EJ2_before << "\t" <<
    endl;
  cout << "after:" << "\t"  <<
    numEvents_passAll << "\t" << numEvents_MB << "\t" <<
    numEvents_DG2 << "\t" << numEvents_EG2 << "\t" <<
  ...
  normalizer->SetBinContent(12, numEvents_DG2);
  ...
  normalizer->GetXaxis()->SetBinLabel(12,"numEvents_DG2");
  normalizer->GetXaxis()->SetBinLabel(13,"numEvents_EG2");
  //normalizer->GetXaxis()->SetBinLabel(14,"numEvents_EJ1");
  //normalizer->GetXaxis()->SetBinLabel(15,"numEvents_EJ2");
  ...
  /*for(int i = 1; i < hMB_E->GetNbinsX()+1; i++)
      ...
      if(numEvents_passAll_MB) 
	{
      ...
      double tempDG2 = contentDG2/((double)numEvents_passAll_DG2*dE);
      double errorDG2 = hDG2_E->GetBinError(i);
      double tempErrDG2 = errorDG2/((double)numEvents_passAll_DG2*dE);
      if(numEvents_passAll_DG2) 
	{
  ...
  hEventCut->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  ...
  hEventCut_MB->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  hEventCut_MB->GetXaxis()->SetBinLabel(7,"passed");

  hEventCut_DG2->GetXaxis()->SetBinLabel(1,"All events");
  hEventCut_DG2->GetXaxis()->SetBinLabel(2,"no GA/GJ trigger");
  ...
  hEventCut_DG2->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  ...
  hEventCut_EG2->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  ...
  hEventCut_EJ1->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  ...
  hEventCut_EJ2->GetXaxis()->SetBinLabel(6,"no 12 GeV cluster");
  ...
  filename += "_cluster_emcalTrigOnly_Allevents_eventCounts_noTrigCuts_12GeVCut_noPileup";
  auto fout = new TFile(Form("isoPhotonOutput/fout_%i_%ibins_%s_GJcuts.root",TrackBit, nbinscluster, filename.Data()), "RECREATE");  
