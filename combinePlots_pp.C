...
void combinePlots_pp(){
  ...
  TFile* file17p1 = new TFile(Form("%sfout_1_26bins_17p_CENT_wSDD_10runs_noThresh_part1_forRTrig_noMBEG2exlusion_Foregroundcc_tof20.root", path.Data()), "READ");
  
  TFile* file365 = new TFile(Form("%sfout_5_26bins_17q_CENT_wSDD_noThresh_r282365_physel_forRTrig_noMBEG2exlusion_Foregroundcc_tof20.root", path.Data()), "READ");
  TFile* file366 = new TFile(Form("%sfout_5_26bins_17q_CENT_wSDD_noThresh_r282366_physel_forRTrig_noMBEG2exlusion_Foregroundcc_tof20.root", path.Data()), "READ");
  TFile* file367 = new TFile(Form("%sfout_5_26bins_17q_CENT_wSDD_noThresh_r282367_physel_forRTrig_noMBEG2exlusion_Foregroundcc_tof20.root", path.Data()), "READ");
  ...
  TH1F* n17p1 = (TH1F*)file17p1->Get("hNormalizer");
  ...
  for(int i = 1; i < hMB_centE->GetNbinsX()+1; i++){
    ...
    hRF->SetBinError(i, 0);
    if(mbContent != 0.000000000000000){
      ...
      cout << i << "\t" << rfContent << "\t" << rfError << "\t" << rfError/rfContent << endl;
      if(rfContent > 1.0 && rfError < 1000){
  ...
  TLegend* lr = new TLegend(0.15, 0.15,0.75, 0.35);
  lr->AddEntry((TObject*)0, "ALICE #sqrt{s_{NN}} = 5.02 TeV pp Trigger Rejection Factors", "");
  lr->AddEntry(hRF, "EG2/MB Fit (10, 40)");

  TCanvas* cr = new TCanvas("cr", "cr");
  cr->SetLogy();
  hRF->GetXaxis()->SetRangeUser(0.0, 40.0);
  hRF->GetYaxis()->SetRangeUser(2e-4, 2e4);
  hRF->Draw("e1");
  lr->Draw("same");
  ...
  hRF->Fit("pol0", "", "", 10, 40);
  hRF->GetFunction("pol0")->SetLineColorAlpha(kBlue, 0.);
  hRF->GetFunction("pol0")->SetLineWidth(5);
